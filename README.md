...
| `ylr1d_hmi` | 人机界面：Qt5 观测 + 控制 | 订阅 `ylr1d_plant` 反馈；控制层链路发期望值给 `ylr1d_control_sim`，转译层链路发 action 给 `ylr1d_translate` |
| `ylr1d_bringup` | 一键启动：聚合各包 launch（无节点） | 聚合除 `ylr1d_description` 外的各包 launch，提供控制层 / 转译层两套链路 |
