#ifndef YLR1D_CONTROL_SIM__CONFIG__JOINT_CONFIG_HPP_
#define YLR1D_CONTROL_SIM__CONFIG__JOINT_CONFIG_HPP_
...
// ── 关节分组（替换节点 cpp 内硬编码，名/顺序与命令数组索引一致） ──
...
/// 机械臂组索引（躯干 / 左臂 / 右臂），与 kArmGroups 数组下标同源。
...
struct ArmGroupDef {
  const char * topic;
  const char * const * joints;
  size_t count;
};

/// 三组机械臂汇总（躯干 4 / 左臂 9 / 右臂 9）。
inline constexpr ArmGroupDef kArmGroups[ARM_GROUP_COUNT] = {
  {kTorsoTopic,    kTorsoJoints,    kTorsoJointCount},
  {kLeftArmTopic,  kLeftArmJoints,  kLeftArmJointCount},
  {kRightArmTopic, kRightArmJoints, kRightArmJointCount},
};

}  // namespace ylr1d_control_sim

#endif  // YLR1D_CONTROL_SIM__CONFIG__JOINT_CONFIG_HPP_
