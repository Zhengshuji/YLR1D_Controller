...
/// 位置闭环：PID + PositionPlant 单位负反馈，阶跃输入。
/// 反馈 = plant.position()；期望 r 阶跃到 target。
...
TEST(ClosedLoop, PositionStepResponse) {
  ...
  double pos = 0.0;
  for (int i = 0; i < n; ++i) {
    double u = pid.compute(target, plant.position(), dt);
    pos = plant.update(u, dt);
  }
  EXPECT_NEAR(pos, target, 0.01);
  EXPECT_LE(plant.velocity(), sp.max_vel + 1e-9);
}

/// 速度闭环：PID + VelocityPlant 单位负反馈，阶跃输入。
/// 反馈 = plant.velocity()；速度型被控对象（1/s）需 PI 消除阶跃静差。
TEST(ClosedLoop, VelocityStepResponse) {
  ...
  double vel = 0.0;
  for (int i = 0; i < n; ++i) {
    double u = pid.compute(target, plant.velocity(), dt);
    vel = plant.update(u, dt);
  }
