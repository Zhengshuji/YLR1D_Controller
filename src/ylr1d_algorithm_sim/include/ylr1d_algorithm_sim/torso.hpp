#ifndef YLR1D_ALGORITHM_SIM__TORSO_HPP_
#define YLR1D_ALGORITHM_SIM__TORSO_HPP_
...
/// torso 组（4 躯干，位置型）协同控制器：组期望 → 逐关节 setpoint。
/// 内部算法：P 比例（control_law/proportional），实现见 src/torso.cpp。
class TorsoCooperativeController final : public CooperativeController {
public:
...
/// torso 组逐关节控制器（每关节一个实例）：setpoint + feedback → u。
/// 内部算法：PID（control_law/pid），实现见 src/torso.cpp。
class TorsoJointController final : public IndependentController {
public:
...
/// torso 组仿真被控对象（位置型，1/s²）：整体输入/整体输出。
/// 内部模型：位置积分器组（control_law/integrator），实现见 src/torso.cpp。
class TorsoPlant final : public GroupPlant {
public:
...
#endif  // YLR1D_ALGORITHM_SIM__TORSO_HPP_
