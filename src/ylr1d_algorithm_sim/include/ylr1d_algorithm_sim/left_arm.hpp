#ifndef YLR1D_ALGORITHM_SIM__LEFT_ARM_HPP_
#define YLR1D_ALGORITHM_SIM__LEFT_ARM_HPP_
...
/// left_arm 组（9 关节，位置型）协同控制器：组期望 → 逐关节 setpoint。
/// 内部算法：P 比例（control_law/proportional），实现见 src/left_arm.cpp。
class LeftArmCooperativeController final : public CooperativeController {
public:
...
/// left_arm 组逐关节控制器（每关节一个实例）：setpoint + feedback → u。
/// 内部算法：PID（control_law/pid），实现见 src/left_arm.cpp。
class LeftArmJointController final : public IndependentController {
public:
...
/// left_arm 组仿真被控对象（位置型，1/s²）：整体输入/整体输出。
/// 内部模型：位置积分器组（control_law/integrator），实现见 src/left_arm.cpp。
class LeftArmPlant final : public GroupPlant {
public:
...
#endif  // YLR1D_ALGORITHM_SIM__LEFT_ARM_HPP_
