#ifndef YLR1D_ALGORITHM_SIM__WHEEL_HPP_
#define YLR1D_ALGORITHM_SIM__WHEEL_HPP_
...
/// wheels 组（4 轮子，速度型）协同控制器：组期望 → 逐关节 setpoint。
/// 内部算法：P 比例（control_law/proportional），实现见 src/wheel.cpp。
class WheelCooperativeController final : public CooperativeController {
public:
...
/// wheels 组逐关节控制器（每关节一个实例）：setpoint + feedback → u。
/// 内部算法：PID（control_law/pid），实现见 src/wheel.cpp。
class WheelJointController final : public IndependentController {
public:
...
/// wheels 组仿真被控对象（速度型，1/s）：整体输入/整体输出。
/// 内部模型：速度积分器组（control_law/integrator），实现见 src/wheel.cpp。
class WheelPlant final : public GroupPlant {
public:
...
private:
  VelocityIntegratorGroup engine_;
};

}  // namespace ylr1d_algorithm_sim

#endif  // YLR1D_ALGORITHM_SIM__WHEEL_HPP_
