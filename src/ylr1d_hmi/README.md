...
│   │   ├── translate_panel.hpp   # TranslatePanel：转译层 action 客户端面板 + ChassisDirectionWidget 方向可视化
...
│   │   └── monitor_nodes.hpp     # 监视面板期望节点清单（ExpectedNode / kExpectedNodes）
│   ├── config/               # 共享静态配置（纯头、全 inline，单一来源）
│   │   ├── joint_defs.hpp        # 关节定义单一来源：JointDef/JointGroup + 30 关节原子集合 + 分组视图
│   │   └── sensor_topics.hpp     # 传感器话题单一来源：相机/点云/雷达/超声/IMU 话题 + 监视用 SensorSpec
│   └── common/               # 共享工具
│       ├── action_sender.hpp     # action goal 发送公共 helper（waitForActionServer / makeGoalOptions 模板）
│       ├── sim_control.hpp       # SimControl：仿真控制服务客户端（暂停 / 继续 / 重置）
...
│   ├── panels/               # 面板实现
│   │   ├── hmi_window.cpp / translate_panel.cpp / sensor_panel.cpp
...
- **关节定义单一来源**：`config/joint_defs.hpp` 统一了控制层 HMI（`hmi_window`）、监视面板（`monitor_panel`）、转译层 HMI（`translate_panel`）三处的关节名/限位/标志（30 关节原子定义 + 4/5/3 组视图），避免口径漂移。**改限位须以 `ylr1d_description/config/limits.yaml` 为基准**同步此文件。
  - 控制层 HMI 各分组限位范围：转向 ±3.14 rad；轮子 ±5.0 rad/s（连续旋转）；躯干 Lift（棱柱）±0.30 m、其余 ±3.14 / ±1.57 rad；左右臂各关节 ±1.57 ~ ±6.28 rad（非对称肩 2 为 -1.57 ~ +1.83）；夹爪（棱柱）左指 [-0.015, 0]、右指 [0, 0.015] m。
